...
void CommandBufferDispatchOp::build(
  ...
  // TODO(scotttodd): Find and use a helper function for this?
  StringRef parentOpSymName =
      entryPoint.getParentOp()
          ->getAttrOfType<StringAttr>(SymbolTable::getSymbolAttrName())
          .getValue();
  ArrayRef<FlatSymbolRefAttr> nestedReferences(
      builder.getSymbolRefAttr(entryPoint));
  state.addAttribute("entry_point", builder.getSymbolRefAttr(parentOpSymName,
                                                             nestedReferences));
}
