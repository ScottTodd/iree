...
struct RewritePseudoCmpNear : public OpRewritePattern<T> {
  ...
  LogicalResult matchAndRewrite(T op,
    ...
    auto comparisonResult = continuationBlock->addArgument(i32Type);
    ...
    auto zero = rewriter.createOrFold<ConstFOp>(op.getLoc(), 0);
    auto lhsPositive =
        rewriter.createOrFold<CmpGTEFOp>(op.getLoc(), i32Type, op.lhs(), zero);
    auto rhsPositive =
        rewriter.createOrFold<CmpGTEFOp>(op.getLoc(), i32Type, op.rhs(), zero);
    auto signsNotEqual = rewriter.createOrFold<IREE::VM::CmpNEI32Op>(
        op.getLoc(), i32Type, lhsPositive, rhsPositive);

    // If signs differ, perform a direct comparison of `lhs == rhs`.
    auto *directComparisonBlock = rewriter.createBlock(continuationBlock);
    auto exactEqual = rewriter.createOrFold<CmpEQFOp>(op.getLoc(), i32Type,
                                                      op.lhs(), op.rhs());
    rewriter.createOrFold<IREE::VM::BranchOp>(op.getLoc(), continuationBlock,
                                              exactEqual);

    // ...else, perform a full ULP-based comparison.
    auto *ulpComparisonBlock = rewriter.createBlock(continuationBlock);
    auto lhsInt =
        rewriter.createOrFold<BitcastFToIOp>(op.getLoc(), i32Type, op.lhs());
    auto rhsInt =
        rewriter.createOrFold<BitcastFToIOp>(op.getLoc(), i32Type, op.rhs());
    auto signedUlpsDiff =
        rewriter.createOrFold<SubIOp>(op.getLoc(), i32Type, lhsInt, rhsInt);
    auto absUlpsDiff =
        rewriter.createOrFold<AbsIOp>(op.getLoc(), i32Type, signedUlpsDiff);
    // The constant chosen here is arbitrary. Higher values increase the
    // distance between arguments that is tolerated.
    auto maxUlpsDiff = rewriter.createOrFold<ConstIOp>(op.getLoc(), 100);
    auto ulpCompare = rewriter.createOrFold<CmpLTIOp>(op.getLoc(), i32Type,
                                                      absUlpsDiff, maxUlpsDiff);
    rewriter.createOrFold<IREE::VM::BranchOp>(op.getLoc(), continuationBlock,
                                              ulpCompare);
    ...
    rewriter.createOrFold<IREE::VM::CondBranchOp>(
        op.getLoc(), signsNotEqual, directComparisonBlock, ulpComparisonBlock);
...
// TODO(scotttodd): something is off here - the folder succeeds on constants
//                  but canonicalization happens anyways and expands to many ops
template <typename T>
...
void CmpEQF64NearOp::getCanonicalizationPatterns(
    OwningRewritePatternList &results, MLIRContext *context) {
