...
static LogicalResult applyAsyncAllocations(Region &region,
  ...
  for (auto *op : ops) {
    ...
    if (failed(TypeSwitch<Operation *, LogicalResult>(op)
                   ...
                   .Case<IREE::Stream::AsyncConcurrentOp>([&](auto op) {
...
static LogicalResult convertAsyncStoreOp(IREE::Stream::AsyncStoreOp asyncOp) {
  auto newOp = OpBuilder(asyncOp).create<IREE::Stream::ResourceStoreOp>(
      asyncOp.getLoc(), asyncOp.value(), asyncOp.target(),
      asyncOp.target_size(), asyncOp.target_offset());
 ...
 public:
  ...
  void runOnOperation() override {
    ...
    for (auto &op :
      ...
      if (failed(TypeSwitch<Operation *, LogicalResult>(&op)
                     .Case<IREE::Stream::AsyncExecuteOp>(
                         [&](auto op) { return allocateExecutionRegion(op); })
                     .Case<IREE::Stream::AsyncLoadOp>(
                         [&](auto op) { return convertAsyncLoadOp(op); })
                     .Case<IREE::Stream::AsyncStoreOp>(
                         [&](auto op) { return convertAsyncStoreOp(op); })
                     .Default(success()))) {
