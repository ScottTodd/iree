...
#define IREE_HAL_WASM_DRIVER_ID 0x5741534D  // WASM

static iree_status_t iree_hal_wasm_driver_factory_enumerate(
    void* self, const iree_hal_driver_info_t** out_driver_infos,
    iree_host_size_t* out_driver_info_count) {
  static const iree_hal_driver_info_t driver_infos[1] = {
      {
          .driver_id = IREE_HAL_WASM_DRIVER_ID,
          .driver_name = iree_string_view_literal("wasm"),
          .full_name = iree_string_view_literal("WebAssembly"),
      },
  };
...
static iree_status_t iree_hal_wasm_driver_factory_try_create(
    void* self, iree_hal_driver_id_t driver_id, iree_allocator_t host_allocator,
    iree_hal_driver_t** out_driver) {
  if (driver_id != IREE_HAL_WASM_DRIVER_ID) {
  ...
  // TODO(scotttodd): Multithreading
  iree_task_topology_t topology;
  iree_task_topology_initialize_from_group_count(1, &topology);

  // TODO(scotttodd): Use task_worker_local_memory flag
  iree_host_size_t worker_local_memory = 64 * 1024;

  iree_status_t status = iree_ok_status();

  iree_hal_executable_loader_t* wasm_loader = NULL;
  if (iree_status_is_ok(status)) {
    status = iree_hal_wasm_module_loader_create(host_allocator, &wasm_loader);
  }
  iree_hal_executable_loader_t* loaders[1] = {wasm_loader};

  iree_task_executor_t* executor = NULL;
  if (iree_status_is_ok(status)) {
    status = iree_task_executor_create(IREE_TASK_SCHEDULING_MODE_RESERVED,
                                       &topology, worker_local_memory,
  ...
  if (iree_status_is_ok(status)) {
    status = iree_hal_task_driver_create(
        iree_make_cstring_view("wasm"), &default_params, executor,
  ...
  iree_hal_executable_loader_release(wasm_loader);
  return status;
}
