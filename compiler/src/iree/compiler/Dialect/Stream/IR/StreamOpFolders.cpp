...
template <typename Op> struct NarrowFillPattern : public OpRewritePattern<Op> {
...
template <typename Op> struct ElideUnusedOp : public OpRewritePattern<Op> {
...
// covering reads.
template <typename Op> struct MaterializeCOW : public OpRewritePattern<Op> {
  using OpRewritePattern<Op>::OpRewritePattern;
  LogicalResult matchAndRewrite(Op op,
                                PatternRewriter &rewriter) const override {
...
//    stream.yield %2
//  }
template <typename Op> struct TieRegionResults : public OpRewritePattern<Op> {
  using OpRewritePattern<Op>::OpRewritePattern;
  LogicalResult matchAndRewrite(Op op,
                                PatternRewriter &rewriter) const override {
