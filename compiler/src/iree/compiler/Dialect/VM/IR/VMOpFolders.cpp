...
template <typename T> static OpFoldResult foldSelectOp(T op) {
...
template <typename T> static OpFoldResult foldSwitchOp(T op) {
  APInt indexValue;
...
template <typename T> static OpFoldResult foldNotOp(T op, Attribute operand) {
...
template <typename T> static OpFoldResult foldCtlzOp(T op, Attribute operand) {
  return constFoldUnaryOp<IntegerAttr>(operand, [](APInt a) {
...
template <typename T> struct EraseUnusedCallOp : public OpRewritePattern<T> {
  using OpRewritePattern<T>::OpRewritePattern;
  LogicalResult matchAndRewrite(T op,
                                PatternRewriter &rewriter) const override {
