...
void buildIREEVMTransformPassPipeline(
  ...
  // signatures.
  IREE_COMPILER_TRACE_ADD_BEGIN_FRAME_PASS(passManager, "Input");
  ...
  IREE_COMPILER_TRACE_ADD_END_FRAME_PASS(passManager, "Input");
  ...
  IREE_COMPILER_TRACE_ADD_BEGIN_FRAME_PASS(passManager, "ABI");
  ...
  IREE_COMPILER_TRACE_ADD_END_FRAME_PASS(passManager, "ABI");
  ...
  switch (schedulingOptions.executionModel) {
    case SchedulingOptions::ExecutionModel::HostOnly:
      ...
      break;
    default:
      IREE_COMPILER_TRACE_ADD_BEGIN_FRAME_PASS(passManager, "Preprocessing");
      IREE::buildPreprocessingPassPipeline(passManager, preprocessingOptions,
                                           hooks.pipelineExtensions);
      IREE_COMPILER_TRACE_ADD_END_FRAME_PASS(passManager, "Preprocessing");
      if (compileTo == IREEVMPipelinePhase::Preprocessing)
        return;  // early-exit

      IREE_COMPILER_TRACE_ADD_BEGIN_FRAME_PASS(passManager, "Flow");
      IREE::Flow::buildFlowTransformPassPipeline(passManager, flowOptions);
      IREE_COMPILER_TRACE_ADD_END_FRAME_PASS(passManager, "Flow");
      if (compileTo == IREEVMPipelinePhase::Flow) return;  // early-exit

      IREE_COMPILER_TRACE_ADD_BEGIN_FRAME_PASS(passManager, "Stream");
      IREE::Stream::buildStreamTransformPassPipeline(passManager,
                                                     streamOptions);
      IREE_COMPILER_TRACE_ADD_END_FRAME_PASS(passManager, "Stream");
  ...
  IREE_COMPILER_TRACE_ADD_BEGIN_FRAME_PASS(passManager, "HAL");
  switch (schedulingOptions.executionModel) {
    case SchedulingOptions::ExecutionModel::HostOnly:
      // No HAL required.
      break;
  ...
  IREE_COMPILER_TRACE_ADD_END_FRAME_PASS(passManager, "HAL");
  if (compileTo == IREEVMPipelinePhase::HAL ||
      halCompileTo != IREE::HAL::PipelinePhase::End) {
    return;  // early-exit
  }

  IREE_COMPILER_TRACE_ADD_BEGIN_FRAME_PASS(passManager, "VM");
  IREE::VM::buildVMTransformPassPipeline(passManager, targetOptions);
  passManager.addPass(IREE::Util::createDropCompilerHintsPass());
  IREE_COMPILER_TRACE_ADD_END_FRAME_PASS(passManager, "VM");
