...
IREE_API_EXPORT iree_status_t
iree_loop_emscripten_using_storage_ctl(void* self, iree_loop_command_t command,
                                       const void* params, void** inout_ptr);
...
static inline iree_loop_t iree_loop_emscripten(iree_status_t* out_status) {
  iree_loop_t loop = {out_status, iree_loop_emscripten_ctl};
  return loop;
}

// Minimum size in bytes required for iree_loop_emscripten_storage_t.
...
#define IREE_LOOP_EMSCRIPTEN_STORAGE_SIZE 512

// Storage for an emscripten loop.
...
typedef iree_alignas(iree_max_align_t) struct iree_loop_emscripten_storage_t {
  uint8_t opaque[IREE_LOOP_EMSCRIPTEN_STORAGE_SIZE];
  iree_status_t status;
} iree_loop_emscripten_storage_t;

// Returns an emscripten loop that uses an external |storage| instead of the
// stack. The storage will only be used while executing and can be reused if the
// caller knows it is safe (not reentrantly inside of a loop execution). Errors
// that arise will be set in the storage status field and must be checked (or
// ignored) by the caller to avoid leaks.
//
// See iree_loop_emscripten for details on the execution behavior.
static inline iree_loop_t iree_loop_emscripten_initialize(
    iree_loop_emscripten_storage_t* storage) {
  storage->status = iree_ok_status();
  iree_loop_t loop = {
      storage,
      iree_loop_emscripten_using_storage_ctl,
  };
  return loop;
}

static void iree_loop_emscripten_deinitialize(
    iree_loop_emscripten_storage_t* storage) {
  if (!storage) return;
