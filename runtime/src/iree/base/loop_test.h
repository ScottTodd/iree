...
TEST_F(LoopTest, Call) {
  ...
  IREE_ASSERT_OK(iree_loop_call(
      ...
      +[](void* user_data_ptr, iree_loop_t loop, iree_status_t status) {
        IREE_TRACE_SCOPE();
        fprintf(stdout, "LoopTest::Call iree_loop_call fn\n");
...
// // should in theory be able to issue calls forever. This test ensures we can
// do
...
// // Tests that a failure will abort other pending tasks, including those
// enqueued
...
//         // didn't correctly abort it. We are enqueuing it reentrantly as a
//         user
...
//   // We need to do this before we issue the wait so that loops which perform
//   the
...
//   iree_wait_source_t resolved_wait_source =
//   iree_event_await(&resolved_event);
